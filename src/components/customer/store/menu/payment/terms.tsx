...
const Terms = ({ isOwner, selectedTerms, storeName }: TermsProps) => {
  ...
  if (!isOwner) {
    ...
    } else if (selectedTerms === "privacyThirdParties") {
      return (
        <article css={wrapper}>
          <h1 css={titleStyle}>{termsTitle}</h1>
          <table css={tableStyle}>
    ...
    if (selectedTerms === "use") {
      ...
      return (
        <article css={[wrapper, ownerListStyle]}>
          <h1 css={titleStyle}>{termsTitle}</h1>
          <h2>
            단골손님은 단골손님-사장님 서비스 제공과 관련하여 아래와 같이 사용자의 개인정보를 수집,
            활용합니다.
          </h2>
          <h2>수집 및 이용항목</h2>
          <ul>
    ...
    } else if (selectedTerms === "marketing") {
      return (
        <article css={[wrapper, ownerListStyle]}>
          <h1 css={titleStyle}>{termsTitle}</h1>
          <h2>마케팅 수집이용동의서</h2>
          ...
          <h2>
            사용자는 개인정보의 수집 및 이용 동의를 거부할 권리가 있습니다. 다만 동의 거부 시
            단골손님-사장님 서비스에서 제공하는 할인, 이벤트 및 이용자 맞춤형 상품 추천 등의 마케팅
            서비스 안내가 제공되지 않습니다.
          </h2>
        </article>
      );
